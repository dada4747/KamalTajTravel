...
public class SwiftLoader: UIView {
    ...
    public class func show(animated: Bool) {
        ...
        let loader = SwiftLoader.sharedInstance
        loader.config.backgroundColor = UIColor.clear //UIColor.black
        loader.config.spinnerColor = UIColor.white
        loader.config.titleTextColor = UIColor(red:0.88, green:0.26, blue:0.18, alpha:1)
        loader.config.spinnerLineWidth = 3.0 //2.0
...
class CommonLoader {
    static let shared = CommonLoader()
    private var imageView: UIImageView?
    private var loadingView: UIView?

    private init() {}

    func startLoader(in view: UIView) {
        guard loadingView == nil else { return } // Loader is already visible
        ...
        // Load GIF image
        if let gifPath = Bundle.main.path(forResource: "alkhaleej-loader", ofType: "gif"),
    ...
    func stopLoader() {
        guard let loadingView = loadingView, let imageView = imageView else { return } // Loader is not visible
