...
class VKKeyboardManager: NSObject {
    ...
    private func create_toolbar() -> Void {
		...
		let space_area = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.flexibleSpace, target: nil, action: nil)
...
extension VKKeyboardManager {
    ...
    @objc private func keyboardWillShow(notification: Notification) -> Void {
		...
		let kbSize: CGSize? = (userInfo![UIResponder.keyboardFrameEndUserInfoKey] as AnyObject).cgRectValue?.size
